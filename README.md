...
| [`dsh-workspace-category-manager`](./dsh-workspace-category-manager) | 0.1.1 | 为 DSH 工作区添加逻辑分类，侧边栏以「分类文件夹 → 项目 → 会话」三级层级展示，支持拖拽归类与排序；兼容 DSH 0.1.6（`uiWorkspace.openSession` 会话切换、`uiSession.sessionStatus` 状态点） |
