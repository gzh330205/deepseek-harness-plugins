...
function CategorySidebar({ api, wide, expandSidebar, t }) {
  ...
  const [addDraft, setAddDraft] = import_react9.default.useState({ mode: "local", path: "", group: "", url: "", branch: "", folder: "", parent: "" });
  ...
  const [addCloning, setAddCloning] = import_react9.default.useState(false);
  const [gitAvailable, setGitAvailable] = import_react9.default.useState(false);
  ...
  const openAddWorkspace = () => {
    setAddDraft({ mode: "local", path: "", group: config.categories[0]?.id ?? "", url: "", branch: "", folder: "", parent: "" });
    setAddFormError("");
    setAddError(null);
    setAddCloning(false);
    setAddOpen(true);
    api.gitImportStatus().then((status) => {
      setGitAvailable(status.available === true);
    }).catch(() => {
  ...
  const browseDirectory = async (field) => {
    ...
    try {
      ...
      if (typeof path === "string" && path !== "") setAddDraft((draft) => ({ ...draft, [field]: path }));
    } catch (error) {
  ...
  const saveAddWorkspace = async () => {
    ...
    setAddFormError("");
    try {
      let path = addDraft.path.trim();
      if (addDraft.mode === "git") {
        const url = addDraft.url.trim();
        if (url === "") throw new Error(t("gitUrlRequired"));
        const parent = addDraft.parent.trim();
        if (parent === "") throw new Error(t("gitParentRequired"));
        setAddCloning(true);
        path = await api.cloneRepository({ url, parentPath: parent, folderName: addDraft.folder.trim(), branch: addDraft.branch.trim() });
      } else if (path === "") {
        return;
      }
  ...
  const modeTab = (mode, label) => (0, import_react10.createElement)("button", { type: "button", className: `wcm-modeTab${addDraft.mode === mode ? " wcm-modeTabOn" : ""}`, disabled: addBusy, onClick: () => {
    setAddDraft((draft) => ({ ...draft, mode }));
    setAddFormError("");
  } }, label);
  const groupField = (0, import_react10.createElement)("label", { className: "wcm-form-field" }, t("chooseGroup"), (0, import_react10.createElement)("select", { className: "wcm-select wcm-selectFull", value: addDraft.group, disabled: addBusy, onChange: (event) => setAddDraft((draft) => ({ ...draft, group: event.target.value })) }, config.categories.map((category) => (0, import_react10.createElement)("option", { key: category.id, value: category.id }, category.name)), (0, import_react10.createElement)("option", { value: "" }, t("uncategorized"))));
  const localFields = [(0, import_react10.createElement)("label", { className: "wcm-form-field", key: "dir" }, t("chooseDirectory"), (0, import_react10.createElement)("div", { className: "wcm-dirRow" }, (0, import_react10.createElement)("input", { className: "wcm-dirInput wcm-localPath", value: addDraft.path, readOnly: true, placeholder: t("directoryPlaceholder") }), (0, import_react10.createElement)("button", { type: "button", disabled: addBusy, onClick: () => browseDirectory("path") }, t("browse"))))];
  const gitFolderName = addDraft.folder.trim() !== "" ? addDraft.folder.trim() : repoNameFromUrl(addDraft.url);
  const gitTarget = addDraft.parent.trim() !== "" && gitFolderName !== "" ? joinDisplayPath(addDraft.parent.trim(), gitFolderName) : "";
  const gitFields = [(0, import_react10.createElement)("label", { className: "wcm-form-field", key: "url" }, t("gitUrl"), (0, import_react10.createElement)("input", { className: "wcm-textInput wcm-gitUrl", value: addDraft.url, disabled: addBusy, placeholder: t("gitUrlPlaceholder"), spellCheck: false, onChange: (event) => setAddDraft((draft) => ({ ...draft, url: event.target.value })) })), (0, import_react10.createElement)("div", { className: "wcm-formFieldRow", key: "row" }, (0, import_react10.createElement)("label", { className: "wcm-form-field" }, t("gitBranch"), (0, import_react10.createElement)("input", { className: "wcm-textInput wcm-gitBranch", value: addDraft.branch, disabled: addBusy, placeholder: t("gitBranchPlaceholder"), spellCheck: false, onChange: (event) => setAddDraft((draft) => ({ ...draft, branch: event.target.value })) })), (0, import_react10.createElement)("label", { className: "wcm-form-field" }, t("gitFolder"), (0, import_react10.createElement)("input", { className: "wcm-textInput wcm-gitFolder", value: addDraft.folder, disabled: addBusy, placeholder: t("gitFolderPlaceholder"), spellCheck: false, onChange: (event) => setAddDraft((draft) => ({ ...draft, folder: event.target.value })) }))), (0, import_react10.createElement)("label", { className: "wcm-form-field", key: "parent" }, t("gitParent"), (0, import_react10.createElement)("div", { className: "wcm-dirRow" }, (0, import_react10.createElement)("input", { className: "wcm-dirInput wcm-gitParent", value: addDraft.parent, readOnly: true, placeholder: t("gitParentPlaceholder") }), (0, import_react10.createElement)("button", { type: "button", disabled: addBusy, onClick: () => browseDirectory("parent") }, t("browse")))), addDraft.url.trim() !== "" && gitFolderName === "" ? (0, import_react10.createElement)("p", { className: "wcm-hint", key: "folderHint" }, t("gitFolderHint")) : null, gitTarget !== "" ? (0, import_react10.createElement)("p", { className: "wcm-hint", key: "target" }, t("gitTargetHint", { path: gitTarget })) : null];
  const addReady = !addBusy && (addDraft.mode === "git" ? addDraft.url.trim() !== "" && addDraft.parent.trim() !== "" : addDraft.path.trim() !== "");
